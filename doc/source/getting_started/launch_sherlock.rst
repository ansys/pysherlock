...
the port specified. If a port is not specified, port ``9090`` is used. ``launch_and_connect`` checks that the requested port is available and

raises ``SherlockCannotUsePortError`` if it is already in use.
