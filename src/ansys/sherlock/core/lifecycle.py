...
class Lifecycle(GrpcStub):
    ...
    def _add_harmonic_profile_entries(self, request, entries):
        """Add the harmonic profile entries to the request."""
        for e in entries:
    ...
    def _add_shock_profile_entries(self, request, entries):
        """Add the shock profile entries to the request."""
        for e in entries:
    ...
    def load_shock_profile_pulses(self, project, phase_name, event_name, file_path):
        """Load shock profile pulses from .dat or .csv file.
        ...
        try:
            if project == "":
                raise SherlockLoadShockProfilePulseError(message="Project name is invalid.")
            if phase_name == "":
                raise SherlockLoadShockProfilePulseError(message="Phase name is invalid.")
            if event_name == "":
                raise SherlockLoadShockProfilePulseError(message="Event name is invalid.")
            if file_path == "":
                raise SherlockLoadShockProfilePulseError(message="File name is invalid.")
